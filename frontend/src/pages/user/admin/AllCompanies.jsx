...
const AllCompanies = () => {
  ...
  const databasetableName = "companies";
    ...
    try {
      ...
      const result = await importExcelData(
        ...
        (result) => {
          ...
          setImportInfo({
            type: 'success',
            message: `Successfully imported ${result.successCount} companies name`
          });
        },
        (error) => {
          setImportError({
      ...
      if (result.successCount > 0 && result.errors?.length > 0) {
        setImportInfo({
          type: 'warning',
          message: `Imported ${result.successCount} companies name with ${result.errors.length} errors`
        });
      }
 ...
 // Set the table name
  ...
  const fetchCompanies = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/profile/admin/all-companies`);
  ...
  const handleDownloadExcel = () => {
    downloadData(companies, 'AllCompanies');
  };
  ...
  const handleDeleteCompany = async () => {
    try {
      const apiUrl = import.meta.env.VITE_API_URL;
      const response = await fetch(
        `${apiUrl}/profile/admin/crud/${tableName}/${companyToDelete.id}`,
        {
          method: "DELETE",
        }
      );
  ...
  return (
    <div className="p-4 max-md:padding-t">
      ...
      {loading ? (
            ...
            >
              ...
              <div className="hidden md:flex flex-col md:flex-row md:items-center justify-center md:w-1/2">
                {editingCompanyId === company.id ? (
                  ...
                  <div className="text-center text-gray-800">{company.name}</div>
                )}
              </div>
        ...
        <div className="text-gray-500 text-center">No companies found.</div>
      )}
